...
exports.create = async (req, res) => {
  try {
    ...
    res.status(500).send({ message: error.message || 'Algún error ha surgido al insertar el dato.', errors: error.errors })
  }
}
...
exports.update = (req, res) => {
  const id = req.params.id
  Contact.update(req.body, { where: { id } }).then(num => {
    if (num == 1) {
...
exports.delete = (req, res) => {
  const id = req.params.id
  Contact.destroy({ where: { id } }).then(num => {
    if (num == 1) {
...
exports.openAIFilter = async (req, res) => {
  let prompt = ' { mysqlGenerator: true, table: contacts, tableStructure: [name,email,subject,message], question: "prompt", config: { selectFields: [name,email,subject], whereDefault: {deletedAt: null}, like: true } } '
  ...
  try {
    ...
    if (bannedWords.test(data)) {
      data = 'SELECT name, email, subject FROM contacts'
    }
    const page = req.query.page || 1
    ...
    const result = await db.sequelize.query(
      data,
      { type: db.sequelize.QueryTypes.SELECT, limit, offset }
    )
    res.status(200).send(result)
  } catch (error) {
    console.log(error)
    res.status(500).send({ message: 'No ha sido posible filtrar.' })
  }
}
